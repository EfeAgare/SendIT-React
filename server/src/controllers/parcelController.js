...
class ParcelController {
    ...
    client
      ...
      .then(result => {
        if (result.rows[0].role === 'user') {
          ...
          res.status(400).json({ message: "Can't create parcel" });
        }
      })
      .catch(err => {
        res.status(500).json(err.message);
    ...
    client
      ...
      .then(result => {
        if (result.rows[0].role === 'user') {
          ...
          client.connect();
          client
            .query(text, [parseInt(req.params.parcelId, 10)])
            .then(result => {
              if (!result.rows[0]) {
                ...
                res
                  .status(400)
                  .json({ message: 'Parcel No longer valid to be cancelled' });
              } else {
                const values = ['cancelled', parseInt(req.params.parcelId, 10)];
                const client = new Client(connectionString);
                client.connect();
                client
                  .query(textUpdate, values)
                  .then(result => {
                    res.status(200).json({
                      success: 'true',
  ...
  static destination(req, res) {
    ...
    client.connect();
    client
      .query(getUser, [req.user.role, req.user.id])
      .then(result => {
        if (result.rows[0].role === 'user') {
          const client = new Client(connectionString);
          client.connect();
