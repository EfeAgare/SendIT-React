...
class UserController {
    ...
    client
      ...
      .then(result => {
        if (result.rows[0]) {
          ...
          const client = new Client(connectionString);
          client.connect();
          client.query(textConfirm, values).then(result => {
    ...
    client
      ...
      .then(result => {
        if (!result.rows[0]) {
          return res
            .status(404)
            .json({ message: 'No account with this email address' });
        }
        if (
          !Helpers.comparePassword(result.rows[0].password, req.body.password)
        ) {
  ...
  static getAUserParcel(req, res) {
    ...
    client.connect();
    client
      .query(getuser, [req.user.id, req.user.role])
      .then(result => {
        if (result.rows[0].role === 'user') {
          const client = new Client(connectionString);
          client.connect();
